    ...
    public static TextureAtlas FromFile(ContentManager content, string fileName)
    {
        ...
        using (Stream stream = TitleContainer.OpenStream(filePath))
        {
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                var sliceSettings = root.Element("SliceSettings");

                if (sliceSettings != null)
                {
                    int tilesWide = int.Parse(sliceSettings.Element("TilesWide")?.Value ?? "1");
                    int tilesHigh = int.Parse(sliceSettings.Element("TilesHigh")?.Value ?? "1");
                    int padding = int.Parse(sliceSettings.Element("Padding")?.Value ?? "0");

                    for (int y = 0; y < tilesHigh; y++)
                    {
                        for (int x = 0; x < tilesWide; x++)
                        {
                            int width = atlas.Texture.Width / tilesWide;
                            int height = atlas.Texture.Height / tilesHigh;
                            string regionName = $"tile_{x}_{y}";
                            atlas.AddRegion(regionName, x * width + padding, y * height + padding, width - padding * 2, height - padding * 2);
                        }
                    }
                }
